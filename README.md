...
> 对不可变 Linux 系统友好的雷神加速器组件：一键伪装 SteamDeck、傻瓜桌面面板。

在 Bazzite / SteamOS / Fedora Atomic 等只读根文件系统上，把雷神加速器（`acc-gw`）跑成可用的"盒子"：
手机 App 绑定 → 游戏加速。解决两个核心痛点——**系统非路由器无 uci/ubus、无 `wlan0` 网卡导致设备不可见**。
...
├── panel/                      # yad 状态面板（零流量探测 + UDP 计数判手机在线）+ desktop 模板
├── LICENSE                     # MIT
└── README.md
```
